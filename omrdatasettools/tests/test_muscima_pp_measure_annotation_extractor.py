...
class MuscimaPlusPlusAnnotationConverterTest(unittest.TestCase):
    ...
    def test_annotation_converter_expect_json_files_to_be_created(self):
        ...
        downloader.download_and_extract_dataset(OmrDataset.MuscimaPlusPlus_V2, self.dataset_directory)
        downloader.download_and_extract_dataset(OmrDataset.MuscimaPlusPlus_MeasureAnnotations,
                                                os.path.join(self.dataset_directory, "v2.0", "data"))
    ...
    @unittest.skip(
        "A measure_separator is duplicated in the data, that does not exist in the image, which causes this test to fail")
